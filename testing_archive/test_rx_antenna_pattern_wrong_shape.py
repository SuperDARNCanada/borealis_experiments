...
"""
Experiment fault: 
    rx_antenna_pattern is the wrong shape
Expected exception:
    Slice .* rx antenna pattern must be the same shape as [beam_angle, antenna_count]
"""
...
def rx_antenna_pattern(beam_angle, freq, antenna_locations):
    ...
    pattern = np.array([1.0 for _ in range(antenna_locations.shape[0])]).reshape((1, antenna_locations.shape[0]))
    return pattern
