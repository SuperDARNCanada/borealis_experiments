...
class ScanboundInterfacingSequence(ExperimentPrototype):
    ...
    def __init__(self):
        ...
        self.add_slice(slice_0)
        slice_1 = copy.deepcopy(slice_0)
        slice_1["scanbound"] = [i * 3.6 for i in range(len(scf.STD_16_FORWARD_BEAM_ORDER))]
        self.add_slice(slice_1, {0: "SEQUENCE"})

    @classmethod
    def error_message(cls):
        return ExperimentException, "Scan boundary not the same between slices 0 and 1 for AVEPERIOD or CONCURRENT interfaced slices"
...
class ScanboundNotIncreasing(ExperimentPrototype):
    ...
    def __init__(self):
        ...
        self.add_slice({
            ...
            "rx_beam_order": scf.STD_16_FORWARD_BEAM_ORDER,
            "tx_beam_order": scf.STD_16_FORWARD_BEAM_ORDER,
            "scanbound": list(reversed([i * 3.5 for i in range(len(scf.STD_16_FORWARD_BEAM_ORDER))])),
            "freq": scf.COMMON_MODE_FREQ_1,
            "acf": True,
        })
